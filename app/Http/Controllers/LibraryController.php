...
class LibraryController extends Controller
{
    ...
    {
        // Start with fetching all books
        $books = Book::query();

        // If a search term is provided, filter the books by the title or any other field
        if ($request->filled('search')) {
            $searchTerm = $request->input('search');
            $books->where('title', 'LIKE', '%' . $searchTerm . '%'); // Adjust field name if needed
        }

        // Execute the query and pass the books to the view

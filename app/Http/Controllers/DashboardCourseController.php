...
class DashboardCourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $latestcourse = Course::create($data);

        $quiz = Quiz::create([
            'course_id' => $latestcourse['id'],
            'title' => 'Sample Quiz for ' . $latestcourse['name'],
            'description' => 'A sample quiz for the course: ' . $latestcourse['name'],
        ]);

        Question::create([
            'quiz_id' => $quiz->id,
            'question_text' => 'Sample Question 1 for Quiz ' . $quiz->id,
        ]);

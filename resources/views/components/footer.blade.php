<div class="bg-primary text-white px-6 py-6">
    <div class="grid grid-cols-1 md:grid-cols-5 text-center items-center border-b border-gray-500 pb-4 gap-4 md:gap-0">

        <!-- Left Column: Social Icons -->
        <div class="flex space-x-4 justify-center items-center col-span-1 md:col-span-2">
            <a href="https://facebook.com" target="_blank"
                class="text-white hover:text-gray-300 transform transition-transform duration-200 hover:scale-110">
                <i class="fab fa-facebook-f text-2xl"></i>
            </a>
            <a href="https://twitter.com" target="_blank"
                class="text-white hover:text-gray-300 transform transition-transform duration-200 hover:scale-110">
                <i class="fab fa-twitter text-2xl"></i>
            </a>
            <a href="https://web.whatsapp.com" target="_blank"
                class="text-white hover:text-gray-300 transform transition-transform duration-200 hover:scale-110">
                <i class="fab fa-whatsapp text-2xl"></i>
            </a>
            <a href="https://instagram.com" target="_blank"
                class="text-white hover:text-gray-300 transform transition-transform duration-200 hover:scale-110">
                <i class="fab fa-instagram text-2xl"></i>
            </a>
        </div>

        <!-- Center Column: Logo -->
        <div class="flex flex-col items-center col-span-1">
            <h1 class="text-2xl font-semibold mb-2">ASOKA</h1>
            <img src="/images/logo.png" alt="Asoka Logo" class="w-16 md:w-24 inline drop-shadow-lg">
        </div>

        <!-- Right Column: Links -->
        <div class="flex flex-col items-center col-span-1 md:col-span-2 space-y-1">
            <a href="/whoweare"
                class="text-white hover:text-gray-300 text-sm transition-colors duration-200">@lang('footer.whoweare')</a>
            <a href="/courses"
                class="text-white hover:text-gray-300 text-sm transition-colors duration-200">@lang('footer.courses')</a>
            <a href="/elibrary"
                class="text-white hover:text-gray-300 text-sm transition-colors duration-200">@lang('footer.elibrary')</a>
            <a href="/newsandupdate"
                class="text-white hover:text-gray-300 text-sm transition-colors duration-200">@lang('footer.news')</a>
            <a href="/gallery"
                class="text-white hover:text-gray-300 text-sm transition-colors duration-200">@lang('footer.gallery')</a>
            <a href="/contactus"
                class="text-white hover:text-gray-300 text-sm transition-colors duration-200">@lang('footer.contact')</a>
        </div>
    </div>

    <!-- Footer Bottom Bar -->
    <div class="flex justify-center items-center pt-4">
        <p class="text-xs text-gray-400 text-center">&copy; 2024 Asoka Center for Buddhist Studies. All rights reserved.
            Powered By
            <a href="https://protechmm.com/" class="hover:underline">ProTechMM</a>
        </p>
    </div>
</div>